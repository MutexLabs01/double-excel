...
const NewsPanel: React.FC<NewsPanelProps> = ({ className = '' }) => {
  ...
  if (loading) {
    return (
      <div className={`bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-700 ${className}`}>
  ...
  return (
    <div className={`bg-gradient-to-b from-gray-900 to-gray-850 rounded-lg shadow-2xl p-6 border border-gray-600 ${className}`}>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-white drop-shadow-md">Latest News</h3>
        <Globe className="h-5 w-5 text-gray-400" />
      </div>
      <div className="space-y-4">
        {news.map((article, index) => (
          <article key={index} className="border-b border-gray-600 pb-4 last:border-b-0">
